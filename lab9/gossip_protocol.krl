ruleset gossip_protocol {
    meta {
        ...
        description <<
            ...
            >>
        author "Jared Huch"
        shares schedule, ownSeenLedger, ownMessageID, getPeer, inNeedOfRumors, peerSubscription, seenLedger, rumorLedger, ids_to_subs, highestSequenceAfterFillingHole, violation_counter
    ...
    global {
        ...
        createTempRumor = function(temp, time) {
        ...
        createThresholdRumor = function(operation, time) { // operation is 1,0,-1
            {"MessageID": ownMessageID(),
             "SensorID": ent:own_id,
             "Operation": operation,
             "Timestamp": time,
        ...
        computeViolationCount = function() {
            rumorArray = ent:rumor_ledger.values().reduce(function(accum, elem) { accum.append(elem)  }) || []
            threshold_rumors = rumorArray.filter(function(rumor) { rumor{"RumorType"} == "threshold" }) || []
            violation_count = threshold_rumors.reduce(function(accum, elem) {accum + elem{"Operation"}}, 0) || 0
            violation_count
        }
    ...
    rule init {
        ...
        always {
            ...
            ent:ids_to_subs := {}
            ent:is_on := true
            ent:last_threshold_operation := -1 // make sure this doesn't end up as a rumor
    ...
    rule react_to_sensor_reading {
        ...
        pre {
            ...
            rumor = createTempRumor(temp, time)
        }
    ...
    rule react_to_violation {
        select when wovyn threshold_violation

        pre {
            operation = (ent:last_threshold_operation == -1) => 1 | 0 // 1 -> 0, 0 -> 0, -1 -> 1
            time = event:attrs{"timestamp"}
            rumor = createThresholdRumor(operation, time);
        }

        always {
            raise gossip event "rumor" attributes {"rumor": rumor}
            ent:message_counter := ent:message_counter + 1
            ent:last_threshold_operation := operation
        }
    }

    rule react_to_not_in_violation {
        select when wovyn not_in_violation where picoOn()

        pre {
            operation = (ent:last_threshold_operation != -1) => -1 | 0 // 1 => -1, 0 => -1, -1 => not fired
            time = event:attrs{"timestamp"}
            rumor = createThresholdRumor(operation, time);
        }

        if ent:last_threshold_operation != -1 then noop() // only send a message on the transition edge from violation -> non-violation
